...
mae_list = []
r2_list = []
n_trees = list(range(100, 201))

for n in n_trees:
    rf = RandomForestRegressor(n_estimators=n, random_state=69, n_jobs=-1)
    ...
    mae_list.append(mae)
    r2_list.append(r2)
    print(f"n_estimators={n}: RMSE={rmse:.2f}, MAE={mae:.2f}, R2={r2:.2f}")

# Mostrar os 3 melhores valores (menor RMSE)
results = list(zip(n_trees, rmse_list, mae_list, r2_list))
results_sorted = sorted(results, key=lambda x: x[1])  # Ordena por RMSE
print("\nTop 3 melhores valores (menor RMSE):")
for i in range(3):
    n, rmse, mae, r2 = results_sorted[i]
    print(f"n_estimators={n}: RMSE={rmse:.2f}, MAE={mae:.2f}, R2={r2:.2f}")

# Plot dos resultados
plt.figure(figsize=(12, 6))
plt.plot(n_trees, rmse_list, marker='o', label='RMSE')
plt.plot(n_trees, mae_list, marker='o', label='MAE')
plt.plot(n_trees, r2_list, marker='o', label='R2')
plt.xlabel('Número de Árvores (n_estimators)')
plt.title('Desempenho do Random Forest vs Número de Árvores (100 a 200)')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
